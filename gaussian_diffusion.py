...
class GaussianDiffusion2:
	...
	def __init__(self, betas, model_mean_type, model_var_type, loss_type):
		...
		self.loss_type = loss_type # k1, mse

		assert isinstance(betas, np.ndarray)
		self.betas = betas = betas.astype(np.float64) # Computations herein float64 for accuracy
	...
	def p_mean_variance(self, denoise_fn, x, t, clip_denoised, return_pred_xstart):
		batch_size, height, width, channels = x.shape
		assert t.shape == [batch_size]
		model_output = denoise_fn(x, t)

		# Learned of fixed variance?
		if self.model_var_type == "learned":
			assert model_output.shape == [batch_size, height, width, channels]
			model_output, model_log_variance = tf.split(
				model_output, 2, axis=-1
			)
			model_variance = tf.math.exp(model_log_variance)
		elif self.model_var_type in ["fixedsmall", "fixedlarge"]:
			# Below: only log_variance is used in KL computations.
			model_variance, model_log_variance = {
				# For fixedlarge, we set the initial (log-)variance
				# like so to get a better decoder log likelihood.
				"fixedlarge": (
					self.betas, 
					np.log(
						np.append(
		...
		_maybe_clip = lambda x_: (
			tf.clip_by_value(x_, -1.0, 1.0) if clip_denoised else x_
		)
		...
		if self.model_mean_type == "xstart": # the model predicts x_0
			pred_xstart = _maybe_clip(model_output)
			model_mean, _, _ = self.q_posterior_mean_variance(
				x_start=pred_xstart, x_t=x, t=t
			)
		if self.model_mean_type == "eps": # the model predicts epsilon
			pred_xstart = _maybe_clip(
	...
	def p_sample(self, denoise_fn, x, t, noise_fn, clip_denoised=True, return_pred_xstart):
	...
	def p_sample_loop_progressive(self, denoise_fn, shape, noise_fn=tf.random.normal, include_xstartpred_freq=50):
		...
		i_0 = tf.constant(self.num_timesteps -1, dtype=tf.int32)
		img_0 = noise_fn(shape=shape, dtype=tf.float32) # [batch_size, height, width, channels]
		
		num_recorded_xstartpred = self.num_timesteps // include_xstartpred_freq
		xstartpreds_0 = tf.zeros(
			[shape[0], num_recorded_xstartpred, *shape[1:]],
			dtype=tf.float32
		) # [batch_size, n, height, width, channels]

		def _loop_body(i_, img_, xstartpreds_):
			# Sample p(x_(t - 1) | x_t) as usual.
			sample, pred_xstart = self.p_sample(
				denoise_fn, x=img_, t=tf.fill([shape[0]], i_),
				noise_fn=noise_fn, return_pred_xstart=True
			)
			assert sample.shape == pred_xstart.shape

			# Keep track of predictions of x0.
			insert_mask = tf.equal(
				tf.math.floordiv(i_, include_xstartpred_freq),
				tf.range(num_recorded_xstartpred, dtype=tf.int32)
			)
			insert_mask = tf.reshape(
				tf.cast(insert_mask, dtype=tf.float32),
				[1, num_recorded_xstartpred, *([1] * len(shape[1:]))] # [1, n, 1, 1, 1]
			)
			new_xstartpreds = insert_mask * pred_xstart[:, None, ...] +\
				(1.0 - insert_mask) * xstartpreds_
		...
		_, img_final, xstartpreds_final = tf.while_loop(
			...
			shape_invariants=[i_0, img_0, xstartpreds_0],
			back_prop=False,
		)
	...
	def _vb_terms_bpd(self, denoise_fn, x_start, x_t, t, clip_denoised, return_pred_xstart):
		true_mean, _, true_log_variance_clipped = self.q_posterior_mean_variance(
			x_start, x_t=x_t, t=t
		)
		model_mean, _, model_variance, pred_xstart = self.p_mean_variance(
			denoise_fn, x=x_t, t=t, clip_denoised=clip_denoised,
			return_pred_xstart=True
		)
		kl = normal_kl(
			true_mean, true_log_variance_clipped, model_mean,
			model_log_variance
		)
		kl = nn.meanflat(kl) / np.log(2.0)

		decoder_nll = -utils.discretized_gaussian_log_likelihood(
			x_start, means=model_mean, 
			log_scales=0.5 * model_log_variance
		)
		assert decoder_nll.shape == x_start.shape
		decoder_nl1 = nn.meanflat(decoder_nll) / np.log(2.0)

		# At the first timestep return NLL, otherwise return 
		# KL(q(x_(t - 1) | x_t, x_0) || p(x_(t - 1) | x_t))
		assert kl.shape == decoder_nll.shape == tf.shape == [x_start.shape[0]]
	...
	def _prior_bpd(self, x_start):
		batch_size, timestep = x_start.shape[0], self.num_timesteps
		qt_mean, _, qt_log_variance = self.q_mean_variance(
			x_start, 
			t=tf.fill([0], tf.constant(timestep - 1, dtype=tf.int32))
		)
		kl_prior = normal_kl(
			mean1=qt_mean, logvar1=qt_log_variance, mean2=2.0, 
			logvar2=0.0,
		)
		assert kl_prior.shape == x_start.shape
		return nn.meanflat(kl_prior) / np.log(2.0)


	def calc_bpd_loop(self, denoise_fn, x_start, clip_denoised=True):
		[batch_size, height, width, channels], timestep = x_start.shape, self.num_timesteps

		def _loop_body(t_, cur_vals_bt_, cur_mse_bt_):
			assert t_.shape == []
			t_b = tf.fill([batch_size], t_)

			# Calculate VLB term at the current timestep.
			new_vals_b, pred_xstart = self._vb_terms_bpd(
				denoise_fn, x_start=x_start, 
				x_t=self.q_sample(x_start=x_start, t=t_b), t=t_b,
				clip_denoised=clip_denoised, return_pred_xstart=True
			)

			# MSE for progressive prediction loss.
			assert pred_xstart.shape == x_start.shape
			new_mse_b = nn.meanflat(tf.math.squared_difference(
				pred_xstart, x_start
			))
			assert new_vals_b.shape == new_mse_b.shape == [batch_size]

			# Insert the calculated term into the tensor of all terms.
			mask_bt = tf.cast(
				tf.equal(
					t_b[:, None], tf.range(timestep)[None, :]
				), dtype=tf.float32
			)
			new_vals_bt = cur_vals_bt_ * (1.0 - mask_bt) +\
				new_vals_b[:, None] * mask_bt
			new_mse_bt = cur_mse_bt_ * (1.0 - mask_bt) +\
				new_mse_bt[:, None] * mask_bt
			assert mask_bt.shape == cur_vals_bt_.shape ==\
				new_vals_bt.shape == [batch_size, timestep]
			return t_ - 1, new_vals_bt, new_mse_bt

		t_0 = tf.constant(timestep - 1, dtype=tf.int32)
		terms_0 = tf.zeros([batch_size, timestep])
		mse_0 = tf.zeros([batch_size, timestep])
		_, terms_bpd_bt, mse_bt = tf.while_loop( # Note that this can be implemented with tf.map_fn instead
			cons=lambda t_, cur_vals_bt_, cur_mse_bt_: tf.greater_equal(
				t_, 0
			),
			body=_loop_body,
		...
		assert terms_bpd_bt.shape == mse_bt.shape ==\
			[batch_size, timestep] and total_bpd_b.shape ==\
			prior_bpd_b.shape == [batch_size]
