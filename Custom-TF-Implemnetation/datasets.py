...
class SimpleDataset:
	...
	def _proc_and_batch(self, ds, batch_size):
		def _process_data(x_):
			...
			return pack(
				image=img_, label=tf.constant(0, dtype=tf.int32)
			)
	...
	def train_input_fn(self, params):
		ds = tfds.load(
			self._tfds_name, split="train", shuffle_files=True,
	...
	def train_one_pass_input_fn(self, params):
		ds = tfds.load(
			self._tfds_name, split="train", shuffle_files=False,
			data_dir=self._data_dir
		)
		return self._proc_and_batch(ds, params["batch_size"])
	...
	def eval_input_fn(self, params):
		...
		ds = tfds.load(
			self._tfds_name, split=self.eval_split_name, 
			shuffle_files=False, data_dir=self._data_dir
		)
		return self._proc_and_batch(ds, params["batch_size"])
...
class LSUNDataset:
	def __init__(self, tfr_file, resolution=256, max_images=None,
		...
		self.tfr_file = tfr_file # path to tfrecord file
		self.dtype = "int32"
		self.max_images = max_images # maximum number of images to use, None = use all images
		self.buffer_mb = buffer_mb # read buffer size (megabytes)
		self.num_classes = 1 # unconditional

		# Determine shape and resolution.
		self.resolution = resolution # dataset resolution
	...
	def _train_input_fn(self, params, one_pass):
		# Build TF expressions.
		dset = tf.data.TFRecordDataset(
			self.tfr_file, compression_type="", 
			buffer_size=self.buffer_mb << 20
		)
		...
		if not one_pass:
			dset = dset.repeat()
		dset = dset.map(
			self._parse_tfrecord_tf, num_parallel_calls=tf.data.AUTOTUNE
		)
	...
	def train_one_pass_input_fn(self, params):
		return self._train_one_pass_input_fn(params, one_pass=True)
	...
	def _parse_tfrecord_tf(self, record):
		# Parse individual image from a tfrecords file into Tensorflow
		# expression.
...
def get_dataset(name, tfds_data_dir=None, tfr_file=None, seed=547):
	# Instantiate a dataset and sets the random seed.
