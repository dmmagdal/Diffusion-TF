...
def conv2d(x, num_units, filter_size=(3, 3), stride=1, dilation=None, 
		padding="same", use_bias=True):
	assert x.shape.ndims == 4
	if isinstance(filter_size, int):
		filter_size = (filter_size, filter_size)
	w = tf.Variable(
		shape=[*filter_size, int(x.shape[-1]), num_units],
		dtype=tf.float32
	)
	x = tf.nn.conv2d(
		x, filter_size=filter_size, strides=stride, padding=padding, 
		dilation=dilation
	)
	return x
...
def get_timestep_embedding(timesteps, embedding_dim):
	...
	assert emb.shape.as_list() == [timesteps.shape[0], embedding_dim]
	return emb